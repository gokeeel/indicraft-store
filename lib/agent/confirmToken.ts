...
// Note: this only verifies signature, expiry, and binding (user/address/cart). The token itself
// isn't marked "used" anywhere — instead, order creation clears the cart inside a Serializable
// transaction (lib/services/orders.ts), so a replayed token's cartHash no longer matches the
// (now-empty) cart and is rejected. That's what actually makes confirmation single-use/double-
// click-safe, not this function.
